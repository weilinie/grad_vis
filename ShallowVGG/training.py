...
import glob
from vgg_shallow import Vgg_shallow
...
def main():
    ...
    if if_train:
        ...
        vgg.init(sess)  # random init for the training
        ...
        for epoch in range(num_epochs):
            ...
            for b in range(int(len(X_train) / bs)):
                ...
                _, train_accu = \
                    sess.run([vgg.train_step, vgg.accuracy],
                             feed_dict={vgg.images: X_train_batch, vgg.labels: Y_train_batch})
            ...
            for b in range(int(len(X_train) / bs)):
                ...
                test_accu += \
                    np.sum(sess.run([vgg.correct_prediction],
                             feed_dict={vgg.images: X_test_batch, vgg.labels: Y_test_batch}))
